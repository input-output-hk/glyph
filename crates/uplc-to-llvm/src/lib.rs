...
pub fn compile(program_str: &str, config: Config) -> Result<Vec<u8>> {
    ...
    let program =
        parser::program(program_str).map_err(|e| CompileError::ParseError(e.to_string()))?;
...
fn generate_llvm_ir(term: &Term<NamedDeBruijn>) -> Result<String> {
    ...
    let temp_dir = tempfile::tempdir().map_err(|e| CompileError::IOError(e))?;
...
"#
    ...
    fs::write(src_dir.join("main.rs"), rust_code).map_err(|e| CompileError::IOError(e))?;
    ...
    if !status.success() {
        return Err(CompileError::CompilationError(
            "Failed to compile Rust code to LLVM IR".to_string(),
        ));
    }
    ...
    if !find_output.status.success() {
        return Err(CompileError::CompilationError(
            "Failed to find LLVM IR file".to_string(),
        ));
    }
    ...
    if llvm_ir_paths.is_empty() || llvm_ir_paths[0].is_empty() {
        ...
        return Err(CompileError::CompilationError(
            "Could not find generated LLVM IR file".to_string(),
        ));
    }
    ...
    let llvm_ir = fs::read_to_string(llvm_ir_path).map_err(|e| CompileError::IOError(e))?;
