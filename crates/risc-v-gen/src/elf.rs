...
//         if !line.starts_with('.') {
//             // Skip directive lines that might change section
//             current_pc += 4;
//         }
//     }
...
//         let section_id =
//             obj.add_section(Vec::new(), section.name.clone().into_bytes(), section_kind);
...
//                             "Failed to parse immediate value in li instruction: {}",
//                             imm_str
//                         )));
//                     },
//                 };
...
//                     Err(e) => {
//                         return Err(CodeGenError::InvalidInstruction(format!(
//                             "Failed to encode li instruction ({}): {:?}",
//                             addi_instr, e
//                         )));
//                     },
//                 }
...
//                     Err(e) => {
//                         return Err(CodeGenError::InvalidInstruction(format!(
//                             "Failed to encode mv instruction ({}): {:?}",
//                             addi_instr, e
//                         )));
//                     },
//                 }
...
//                         Err(e) => {
//                             return Err(CodeGenError::InvalidInstruction(format!(
//                                 "Failed to encode la instruction ({}): {:?}",
//                                 addi_instr, e
//                             )));
//                         },
//                     }
//                 } else {
//                     return Err(CodeGenError::InvalidInstruction(format!(
//                         "Symbol not found for la instruction: {}",
//                         symbol
//                     )));
//                 }
//                 continue;
...
// ) -> Result<(Vec<(String, u32)>, HashMap<String, u64>)> {
...
//         },
//         Instruction::Li(rd, imm) => Some(format!("li {}, {}", rd.name(), imm)),
...
//         },
//         Instruction::Jal(rd, label) => Some(format!("jal {}, {}", rd.name(), label)),
//         Instruction::Ecall => Some("ecall".to_string()),

//         // Add support for Mv instruction (which is addi rd, rs, 0)
//         Instruction::Mv(rd, rs) => Some(format!("mv {}, {}", rd.name(), rs.name())),

//         // Add support for La instruction
//         Instruction::La(rd, symbol) => Some(format!("la {}, {}", rd.name(), symbol)),
...
//         Instruction::J(label) => Some(format!("j {}", label)),
...
//                     Ok(None) => Err(CodeGenError::InvalidInstruction(format!(
//                         "Failed to encode instruction: {}",
//                         addi_asm
//                     ))),
//                     Err(_) => {
...
//             } else {
//                 return Err(CodeGenError::InvalidInstruction(format!(
//                     "Label not found for la instruction: {}",
//                     symbol
//                 )));
//             }
//         },
...
//                 return Err(CodeGenError::InvalidInstruction(format!(
//                     "Immediate out of range for addi: {}",
//                     imm
//                 )));
//             }
...
//                 if offset % 4 != 0 {
//                     return Err(CodeGenError::InvalidInstruction(format!(
//                         "Branch target not aligned to 4 bytes: {}",
//                         label
//                     )));
//                 }
...
//                 let instr = (imm_12 | imm_10_5 | (rs2_value << 20) | (rs1_value << 15))
//                     | imm_4_1
//                     | imm_11
//                     | 0b1100011;
...
//                 let memory_block =
//                     &input[memory_block_start..memory_block_start + memory_block_end + 1];
//                 script.memory_regions = parse_memory_regions(memory_block)?;
//             } else {
//                 return Err(CodeGenError::InvalidInstruction(
//                     "Missing closing brace for MEMORY block".to_string(),
//                 ));
//             }
//         }
...
//             } else {
//                 return Err(CodeGenError::InvalidInstruction(
//                     "Missing closing parenthesis for ENTRY".to_string(),
//                 ));
//             }
//         }
...
//                 return Err(CodeGenError::InvalidInstruction(format!(
//                     "Memory region origin out of range: 0x{:X}",
//                     region.origin
//                 )));
//             }
...
//         if input.contains(".text : { *(.text) } > ROM")
//             && input.contains(".data : { *(.data) } > RAM")
//             && input.contains(".bss")
//         {
...
//                 },
//             ];
//         } else if input.contains(".text : ALIGN(4)")
//             && input.contains(".rodata : ALIGN(8)")
//             && input.contains(".data : ALIGN(8)")
//             && input.contains(".bss : ALIGN(8)")
//         {
...
//                     let sections_block =
//                         &input[sections_block_start..sections_block_start + sections_block_end + 1];
//                     script.sections = parse_sections(sections_block, &script.memory_regions)?;
//                 } else {
//                     return Err(CodeGenError::InvalidInstruction(
//                         "Missing closing brace for SECTIONS block".to_string(),
//                     ));
//                 }
//             }
//         }
...
//             let region_exists = script
//                 .memory_regions
//                 .iter()
//                 .any(|region| region.name == section.memory_region);
//             if !region_exists && !section.memory_region.is_empty() {
//                 return Err(CodeGenError::InvalidInstruction(format!(
//                     "Section '{}' references undefined memory region '{}'",
//                     section.name, section.memory_region
//                 )));
//             }
//         }
...
//             CodeGenError::InvalidInstruction(format!("Failed to read linker script: {}", e))
//         })?;
//         Self::parse(&content)
//     }
// }
...
//         .ok_or_else(|| {
//             CodeGenError::InvalidInstruction("Invalid MEMORY block format".to_string())
//         })?;
...
//                 let origin_end = line[origin_start..]
//                     .find(',')
//                     .unwrap_or(line.len() - origin_start);
...
//                 let length_str = line[length_start..].trim().trim_end_matches([',', '}']);
...
//         if origin_negative {
//             return Err(CodeGenError::InvalidInstruction(format!(
//                 "Negative origin value in memory region '{}'",
//                 name
//             )));
//         }
...
//         .and_then(|s| s.trim().strip_suffix('}'))
//         .ok_or_else(|| {
//             CodeGenError::InvalidInstruction("Invalid SECTIONS block format".to_string())
//         })?;
...
//                         {
//                             let align_str = section_decl
//                                 [align_pos + open_paren + 1..align_pos + open_paren + close_paren]
//                                 .trim();
...
//                 current_section_data.extend_from_slice(value.as_bytes());
//             },
...
//             .parse::<u64>()
//             .map(|v| v * 1024)
//             .map_err(|_| CodeGenError::InvalidInstruction(format!("Invalid size: {}", size_str)))
