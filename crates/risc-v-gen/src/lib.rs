...
impl BitVMXInstruction {
    ...
    pub fn new(instruction: Instruction, pc: u32, segment_type: MemorySegmentType) -> Self {
        ...
        match &instruction {
            ...
            Instruction::Lui(rd, _imm) | Instruction::Auipc(rd, _imm) => {
                write_addr = Some(rd.number() as u32); // Register write
                write_value = Some(0); // Placeholder
                last_modified_addr = write_addr;
            },
    ...
    pub fn format_for_bitvm(&self) -> String {
        ...
        format!(
            ...
            self.opcode_addr, read1, read2, write, self.next_pc, self.segment_type
        )
    }
...
impl BitVMXCodeGenerator {
    ...
    pub fn add_instruction(
    ...
    ) -> std::result::Result<(), BitVMXCodeGenError> {
        ...
        let bitvm_instruction =
            BitVMXInstruction::new(instruction.clone(), self.pc, self.segment_type);
    ...
    pub fn validate_memory_access(
        ...
        match memory::is_valid_memory_operation(
            address,
            self.segment_type == MemorySegmentType::ReadWrite,
            alignment,
        ) {
    ...
    pub fn add_instruction_with_validation(
        &mut self,
        instruction: Instruction,
    ) -> std::result::Result<(), BitVMXCodeGenError> {
        // Extract memory address from the instruction
        let address = match &instruction {
            Instruction::Lw(_, offset, _base)
            | Instruction::Lh(_, offset, _base)
            | Instruction::Lb(_, offset, _base)
            | Instruction::Lhu(_, offset, _base)
            | Instruction::Lbu(_, offset, _base)
            | Instruction::Sw(_, offset, _base)
            | Instruction::Sh(_, offset, _base)
            | Instruction::Sb(_, offset, _base) => {
