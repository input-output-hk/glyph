...
fn main() {
    ...
    //     let uplc_program =
    //         fs::read_to_string("examples/simple.uplc").expect("Failed to read UPLC program");

    //     // Create a compiler with BitVMX compatibility
    //     let compiler = Compiler::new().with_optimization_level(OptimizationLevel::None);

    //     // Compile the UPLC program to RISC-V assembly
    //     let assembly = compiler
    //         .compile(&uplc_program)
    //         .expect("Failed to compile UPLC program");

    //     // Write the assembly to a file
    //     fs::write("examples/simple.s", &assembly).expect("Failed to write assembly to file");
    ...
    //     fs::write("examples/simple_trace.csv", trace.to_csv()).expect("Failed to write trace to file");
    ...
    // fn generate_execution_trace(assembly: &str) -> ExecutionTrace {
