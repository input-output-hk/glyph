...
pub fn expected_to_program(expected_file: &PathBuf) -> Result<Program<Name>, String> {
...
pub fn actual_evaluation_result(
    file: &Path,
    language: &Language,
...
fn plutus_conformance_tests(language: Language) {
    ...
    for entry in WalkDir::new(root).into_iter().filter_map(|e| e.ok()) {
        ...
        if path.extension().and_then(OsStr::to_str) == Some("uplc") {
            let _expected_file = path.with_extension("uplc.expected");
            let _expected_budget_file = path.with_extension("uplc.budget.expected");
