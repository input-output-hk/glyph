...
use crate::constants::{bool_val, const_tag, data_tag, header, int_size, memory_region, term_tag};
...
impl<'a> UPLCSerializer<'a> {
    ...
    fn write_header(&mut self) -> Result<()> {
        ...
        let root_addr = self.root_term_address
            .ok_or_else(|| SerializationError::MemoryLayoutError(
                "Root term address not set".to_string()
            ))?;
    ...
    fn serialize_term(&mut self, term: &Term<DeBruijn>) -> Result<Address> {
        ...
        match term {
            ...
            Term::Lambda { parameter_name: _, body } => self.serialize_lambda(body)?,
    ...
    fn serialize_apply(&mut self, function: &Rc<Term<DeBruijn>>, argument: &Rc<Term<DeBruijn>>) -> Result<()> {
        // Tag byte
        ...
        self.output.write_u32::<LittleEndian>(function_addr.as_u32())?;
        self.output.write_u32::<LittleEndian>(argument_addr.as_u32())?;
    ...
    fn serialize_constant(&mut self, constant: &Rc<Constant>) -> Result<()> {
        ...
        match &**constant {
            ...
            _ => return Err(SerializationError::InvalidTermType(
    ...
    fn serialize_integer_constant(&mut self, int: &num_bigint::BigInt) -> Result<()> {
        ...
        if int.bits() <= 8 && int >= &num_bigint::BigInt::from(-128) && int <= &num_bigint::BigInt::from(127) {
            ...
            if let Some(i) = int.to_i8() {
                ...
                return Err(SerializationError::IntegerTooLarge(
                    format!("Integer doesn't fit in i8: {}", int)
                ));
            }
        } else if int.bits() <= 16 && int >= &num_bigint::BigInt::from(-32768) && int <= &num_bigint::BigInt::from(32767) {
            ...
            if let Some(i) = int.to_i16() {
                ...
                return Err(SerializationError::IntegerTooLarge(
                    format!("Integer doesn't fit in i16: {}", int)
                ));
            }
        } else if int.bits() <= 32 && int >= &num_bigint::BigInt::from(i32::MIN) && int <= &num_bigint::BigInt::from(i32::MAX) {
            ...
            if let Some(i) = int.to_i32() {
                ...
                return Err(SerializationError::IntegerTooLarge(
                    format!("Integer doesn't fit in i32: {}", int)
                ));
            }
        } else if int.bits() <= 64 && int >= &num_bigint::BigInt::from(i64::MIN) && int <= &num_bigint::BigInt::from(i64::MAX) {
            ...
            if let Some(i) = int.to_i64() {
                ...
                return Err(SerializationError::IntegerTooLarge(
                    format!("Integer doesn't fit in i64: {}", int)
                ));
            }
        } else {
            ...
            let sign_byte = if sign == num_bigint::Sign::Minus { 0x01 } else { 0x00 };
            ...
            if bytes.len() > u32::MAX as usize {
                return Err(SerializationError::IntegerTooLarge(
                    format!("BigInt too large: {} bytes", bytes.len())
                ));
            }
    ...
    fn serialize_bytestring_constant(&mut self, bytes: &[u8]) -> Result<()> {
        ...
        if bytes.len() > u32::MAX as usize {
            return Err(SerializationError::ByteStringTooLarge(
                format!("ByteString too large: {} bytes", bytes.len())
            ));
        }
    ...
    fn serialize_string_constant(&mut self, s: &str) -> Result<()> {
        ...
        if bytes.len() > u32::MAX as usize {
            return Err(SerializationError::StringTooLarge(
                format!("String too large: {} bytes", bytes.len())
            ));
        }
    ...
    fn serialize_bool_constant(&mut self, value: bool) -> Result<()> {
        ...
        self.output.write_u8(if value { bool_val::TRUE } else { bool_val::FALSE })?;
    ...
    fn serialize_case(&mut self, constr: &Rc<Term<DeBruijn>>, branches: &[Term<DeBruijn>]) -> Result<()> {
        // Tag byte
        ...
        // Case count (16-bit)
        self.output.write_u16::<LittleEndian>(branches.len() as u16)?;
        
        // Write constructor address
        self.output.write_u32::<LittleEndian>(constr_addr.as_u32())?;
