...
fn makeBoolConst(m: *Machine, value: bool) *const Value {
    var result = m.heap.createArray(u32, 4);
    result[0] = 1;
    result[1] = @intFromPtr(ConstantType.booleanType());
    result[2] = @intFromPtr(result + 3);
...
pub fn verifyEd25519Signature(m: *Machine, args: *LinkedValues) *const Value {
...
pub fn verifyEcdsaSecp256k1Signature(m: *Machine, args: *LinkedValues) *const Value {
    const signature_bs = args.value.unwrapBytestring();
    const message_bs = args.next.?.value.unwrapBytestring();
    const public_key_bs = args.next.?.next.?.value.unwrapBytestring();
...
pub fn verifySchnorrSecp256k1Signature(m: *Machine, args: *LinkedValues) *const Value {
    const signature_bs = args.value.unwrapBytestring();
    const message_bs = args.next.?.value.unwrapBytestring();
    const public_key_bs = args.next.?.next.?.value.unwrapBytestring();
